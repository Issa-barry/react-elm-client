...
// Maintenir le splash natif jusqu'à la fin du check d'auth
...
export default function Index() {
  useEffect(() => {
    async function bootstrap() {
      ...
      // Token valide → app principale, sinon → login
      router.replace(token ? '/(tabs)' : '/(auth)/login');
    }
    bootstrap();
  }, []);

  // Écran de chargement React Native (visible pendant hideAsync + navigation)
  return (
    <View style={styles.container}>
      <View style={styles.logoBox}>
        <Svg width={80} height={80} viewBox="0 0 32.25 32" fill="none">
          <Path
            fillRule="evenodd"
            clipRule="evenodd"
            d={LOGO_PATH}
